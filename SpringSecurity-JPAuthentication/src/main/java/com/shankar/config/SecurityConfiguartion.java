...
public void configure(HttpSecurity http) throws Exception {
	...
	.antMatchers("/bank/balance").hasAnyAuthority("CUSTOMER","MANAGER","VISTOR")//authentiaction+authorization for customer,manager role
	.antMatchers("/bank/approve").hasAnyRole("MANAGER")//authentication+authorization for manager role
	.anyRequest().authenticated()
	.and().formLogin().
	loginPage("/user/showLogin").
	loginProcessingUrl("/login").
	failureUrl("/user/showLogin?error").defaultSuccessUrl("/bank/", true)
	.and().rememberMe()
	//.and().httpBasic()
